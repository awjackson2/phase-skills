...
This file provides guidance to any agent working in this repository. It is
mirrored byte-for-byte as `CLAUDE.md`, `AGENTS.md`, and `AGENT.md`, so Claude
Code and ChatGPT/Codex receive identical rules. After editing this guide, copy
it to the other two.
...
- `TERMINOLOGY.md` — the glossary. Defines the **Major / Minor / Patch** hierarchy, the capitalization rule (always capitalize them as phase nouns), "start/close the phase" meaning the whole process, the **knowledge-bundle terms** (Knowledge bundle, Concept, Evidence, Relationship), and the **scoped phase recap** rules (a recap reports the prior phases *within the current phase's scope*).
...
- `phase_project.md` — the **portable workflow charter**: a self-sufficient markdown block meant to be pasted into any project's agent guide (`CLAUDE.md` / `AGENTS.md`, fresh or existing) to integrate the workflow. `phase-project-init` and `phase-adopt` install it as the guide's "Phase Workflow" section, reading it from the **shipped asset copy** `phase-project-init/assets/phase_project.md` — the root file never reaches an installed project, so the asset must stay byte-identical to it (`validate_repo.py` enforces this; re-copy after any charter edit). Keep it in sync with the above when conventions change.
...
- `phase-project-init/SKILL.md` — bootstraps a **fresh/empty** project: the `development/` knowledge bundle, the templates, conventions, and — only if the user says yes when asked — git. **Git is optional across the workflow**: init must ask rather than assume, a no-git project skips the branch/worktree/commit/PR steps of the cycle, and adding git later is a documented one-step upgrade. Carries its installable payload under `phase-project-init/assets/` — the `development/` bundle, the portable charter (`phase_project.md`), and the PR template (`.github/`); anything an install step needs must live there, because the repo-root docs never ship with the skills.
...
- **phase-tracker writes** the artifacts (`phase_<NUM>_plan.md`, `phase_<NUM>_log.md`, `phase_index.md`); **phase-recap reads** them. Conventions defined in one are relied on by the other. Two that recur and must agree across both files:
  ...
  - **`development/` is a knowledge bundle.** Everything inside it is a Concept with YAML front matter; phase records also end with one `## OKF relationships` footer. Evidence (audits, triage, scratch notes) lives outside it. The OKF format is **not packaged separately** — no schema file, no validator script ships with the skills; the templates carry the shape and the agent upholds it. `phase-tracker` authors conforming records, `phase-audit` checks them by reading (check 11), `phase-recap` reads the front matter instead of parsing prose, and init/adopt install the payload.
...
- Three skills write into `development/phase_log/` (tracker, decompose, and audit's fixes); the rest read. In a git-tracked project, all writes except init's one bootstrap commit go through a branch + PR; in a project that declined git, they are plain file writes.
