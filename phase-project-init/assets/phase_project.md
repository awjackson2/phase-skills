...
Copy everything below the line into any project's agent guide — CLAUDE.md,
AGENTS.md, or both, fresh or existing — to make the agent follow the phase-log
workflow. It is self-sufficient: it drives the behavior even if the phase-*
skills aren't installed. If they ARE installed, the "Skills" section says which
to use when, and they add the full detail.

This charter ships with the skills as phase-project-init/assets/phase_project.md
so init/adopt can install it in any project; that asset must stay byte-identical
to this repo-root canonical copy (scripts/validate_repo.py enforces it). Keep
this block in sync with TERMINOLOGY.md and the templates/ when conventions
change.
-->
