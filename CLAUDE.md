...
- `phase-project-init/SKILL.md` — bootstraps a **fresh/empty** project: git, the `development/` knowledge bundle, the templates, the `scripts/okf/` validator and schema, conventions. Carries its installable payload under `phase-project-init/assets/` (both `development/` and `scripts/okf/`).
...
- **phase-tracker writes** the artifacts (`phase_<NUM>_plan.md`, `phase_<NUM>_log.md`, `phase_index.md`); **phase-recap reads** them. Conventions defined in one are relied on by the other. Two that recur and must agree across both files:
  ...
  - **`development/` is a knowledge bundle.** Everything inside it is a Concept with YAML front matter; phase records also end with one `## OKF relationships` footer. Evidence (audits, triage, scratch notes) lives outside it. `scripts/okf/manage_bundle.py` — stdlib only, so it runs in a project that has installed nothing — enforces the contract in `scripts/okf/profile.md`. `phase-tracker` authors conforming records, `phase-audit` runs the validator, `phase-recap` reads the front matter instead of parsing prose, and init/adopt install the whole payload.
